...
import { useCustomerReviewImageUpload } from '~/composables/useCustomerReviewImageUpload'
...
const reviewIdRef = toRef(() => props.reviewId)
const { uploading, uploadError, pendingPreviewUrl, selectFile, removeImage, flushPending } =
  useCustomerReviewImageUpload(reviewIdRef)
...
async function onRemove() {
  if (!displayUrl.value || !confirm('ลบรูปรีวิว?')) return
...
<template>
  <div class="space-y-2">
    ...
    <p class="text-xs text-slate-500">การ์ดรีวิวเต็มใบ — แนวตั้ง 3:4 แนะนำ</p>
    ...
    >
      ...
      <div
        v-else
        class="flex aspect-[3/4] w-full flex-col items-center justify-center px-4 py-8 text-center text-sm text-slate-500"
      >
