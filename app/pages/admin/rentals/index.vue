...
const rentals = ref<RentalRequestListItem[]>([])
...
const detailRental = ref<RentalRequestListItem | null>(null)

const rangeLabel = computed(() => {
  if (total.value === 0) return 'ไม่พบรายการ'
  const from = (page.value - 1) * RENTAL_LIST_PAGE_SIZE + 1
  const to = Math.min(page.value * RENTAL_LIST_PAGE_SIZE, total.value)
...
function buildQuery() {
  const q: Record<string, string> = {
    ...
    page_size: String(RENTAL_LIST_PAGE_SIZE),
  }
  if (filters.min_rent) q.min_rent = filters.min_rent
  if (filters.max_rent) q.max_rent = filters.max_rent
...
async function load() {
  ...
  try {
    const res = await $fetch<{
      rentals: RentalRequestListItem[]
      total: number
      page: number
      total_pages: number
    }>('/api/admin/rentals', { query: buildQuery() })
    rentals.value = res.rentals
    total.value = res.total
    page.value = res.page
    totalPages.value = res.total_pages
    if (detailRental.value) {
      const fresh = res.rentals.find(r => r.id === detailRental.value!.id)
      detailRental.value = fresh ?? null
    }
  } catch (e: unknown) {
...
function setStatusTab(status: RentalRequestStatus) {
...
function openDetail(row: RentalRequestListItem) {
  detailRental.value = row
}
...
function createdByText(row: RentalRequestListItem) {
...
function handledByText(row: RentalRequestListItem) {
...
async function patchStatus(
  row: RentalRequestListItem,
  status: RentalRequestStatus,
  event?: Event,
) {
  event?.stopPropagation()
  const confirmMessages: Record<RentalRequestStatus, string> = {
  ...
  try {
    await $fetch(`/api/admin/rentals/${row.id}/status`, { method: 'PATCH', body: { status } })
    const successMessages: Record<RentalRequestStatus, string> = {
...
async function onDelete(row: RentalRequestListItem, event?: Event) {
  event?.stopPropagation()
  if (row.status !== 'rejected') return
  if (!confirm(`ลบคำขอของ ${row.customer_name} ออกจากระบบถาวร?`)) return
  actionId.value = row.id
  try {
    await $fetch(`/api/admin/rentals/${row.id}`, { method: 'DELETE' })
    successMessage.value = `ลบรายการของ ${row.customer_name} แล้ว`
    closeDetail()
    if (rentals.value.length <= 1 && page.value > 1) page.value--
    await load()
  } catch (e: unknown) {
    const err = e as { data?: { statusMessage?: string } }
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-wrap gap-1 rounded-lg border border-slate-200 bg-white p-1">
      <button
        v-for="st in RENTAL_STATUS_TABS"
        :key="st.id"
        type="button"
        class="rounded-md px-3 py-1.5 text-sm font-medium transition"
        :class="filters.status === st.id ? 'bg-emerald-700 text-white' : 'text-slate-600 hover:bg-slate-50'"
    ...
    <div class="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <div v-else-if="rentals.length === 0" class="p-12 text-center text-slate-500">
        ไม่พบรายการ
      </div>
      <table v-else class="w-full min-w-[640px] text-left text-sm">
        <thead class="border-b border-slate-200 bg-slate-50 text-slate-600">
          <tr>
            ...
            <th class="px-4 py-3 font-medium text-right">ช่วงราคาเช่า</th>
        ...
        <tbody class="divide-y divide-slate-100">
          <tr
            ...
            :key="row.id"
            class="cursor-pointer text-slate-800 transition hover:bg-emerald-50/50"
          ...
          >
            ...
            <td class="px-4 py-3 text-right font-medium tabular-nums">
              {{ rentalBudgetText(row.rent_budget_min, row.rent_budget_max) }}
            </td>
            <td
            ...
            <td class="px-4 py-3" @click.stop>
              <div class="flex items-center justify-center gap-1">
                ...
                <button
                  type="button"
                  class="rounded-lg p-1.5 text-emerald-700 hover:bg-emerald-50"
                  title="ดูรายละเอียด"
                  @click="openDetail(row)"
                >
                  <svg class="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    >
      ...
      <button
        ...
        :class="pn === page ? 'bg-emerald-700 text-white' : 'border border-slate-300'"
        @click="goToPage(pn)"
      >
        {{ pn }}
      </button>
    ...
    >
      <div class="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
        <h3 class="mb-4 text-lg font-semibold">กรองคำขอเช่า</h3>
        <div class="space-y-4">
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              <label class="mb-1 block text-sm font-medium">ราคาเช่าต่ำสุด</label>
              <input v-model="draftFilters.min_rent" type="number" min="0" class="w-full rounded-lg border px-3 py-2 text-sm">
            </div>
            <div>
              <label class="mb-1 block text-sm font-medium">ราคาเช่าสูงสุด</label>
              <input v-model="draftFilters.max_rent" type="number" min="0" class="w-full rounded-lg border px-3 py-2 text-sm">
            </div>
          </div>
        ...
        <div class="mt-6 flex justify-end gap-2">
          ...
          <button
            ...
            @click="filters.min_rent = draftFilters.min_rent; filters.max_rent = draftFilters.max_rent; filters.province = draftFilters.province; filters.district = draftFilters.district; filters.subdistrict = draftFilters.subdistrict; filterDialogOpen = false; page = 1; load()"
          >
            ค้นหา
          </button>
        </div>
    ...
    >
      ...
      >
        <div class="mb-4 flex items-start justify-between gap-3">
          <div>
            <h3 id="rental-detail-title" class="text-lg font-semibold text-slate-900">
              {{ detailRental.customer_name }}
            </h3>
            <div class="mt-1 flex flex-wrap items-center gap-2">
              <RentalRequestStatusBadge :status="detailRental.status" />
              <LoanCreatedSourceBadge :source="detailRental.created_source" />
            </div>
          </div>
          <button
            type="button"
        ...
        <dl class="space-y-3 text-sm">
          <div class="grid grid-cols-[8rem_1fr] gap-2">
            ...
            <dd>{{ detailRental.callback_phone }}</dd>
          </div>
          <div class="grid grid-cols-[8rem_1fr] gap-2">
            <dt class="text-slate-500">ไลน์ / ติดต่อ</dt>
            <dd>{{ detailRental.callback_line }}</dd>
          </div>
          <div class="grid grid-cols-[8rem_1fr] gap-2">
            <dt class="text-slate-500">ช่วงราคาเช่า</dt>
            <dd class="font-medium">
              {{ rentalBudgetText(detailRental.rent_budget_min, detailRental.rent_budget_max) }}
            </dd>
          </div>
          <div class="grid grid-cols-[8rem_1fr] gap-2">
          ...
          <div class="grid grid-cols-[8rem_1fr] gap-2 border-t border-slate-100 pt-3">
            ...
            <dd>
              {{ createdByText(detailRental) }}
              <span v-if="detailRental.created_source === 'admin' && detailRental.created_by" class="block text-xs text-slate-400">
                user: {{ detailRental.created_by }}
              </span>
            </dd>
          </div>
          <div
            v-if="detailRental.handled_by"
            class="grid grid-cols-[8rem_1fr] gap-2"
          >
            <dt class="text-slate-500">จัดการโดย</dt>
            <dd>
              {{ handledByText(detailRental) }}
              <span v-if="detailRental.handled_at" class="block text-xs text-slate-400">
                {{ formatHandledAt(detailRental.handled_at) }}
              </span>
            </dd>
          </div>
          <div class="grid grid-cols-[8rem_1fr] gap-2 text-xs text-slate-400">
            <dt>สถานะ</dt>
            <dd>{{ rentalStatusLabel(detailRental.status) }}</dd>
          </div>
        </dl>

        <div class="mt-6 flex flex-wrap gap-2 border-t border-slate-100 pt-4">
          <NuxtLink
            v-if="detailRental.status === 'pending_approval'"
            :to="`/admin/rentals/${detailRental.id}/edit`"
            class="flex-1 rounded-lg border border-slate-300 py-2 text-center text-sm hover:bg-slate-50"
            @click="closeDetail"
          >
            แก้ไขข้อมูล
          </NuxtLink>
          <template v-if="detailRental.status === 'pending_approval'">
            <button
              type="button"
              class="rounded-lg p-2 text-green-700 hover:bg-green-50 disabled:opacity-40"
              title="ติดต่อแล้ว"
              :disabled="actionId === detailRental.id"
              @click="patchStatus(detailRental, 'completed')"
            >
            ...
            <button
              ...
              title="ไม่อนุมัติ"
              :disabled="actionId === detailRental.id"
              @click="patchStatus(detailRental, 'rejected')"
            >
              <svg class="h-7 w-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
          ...
          <button
            v-else
            type="button"
            class="flex items-center gap-1.5 rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-2 text-sm font-medium text-emerald-900 hover:bg-emerald-100 disabled:opacity-40"
            title="ย้อนกลับเป็นรอดำเนินการ"
            :disabled="actionId === detailRental.id"
            @click="patchStatus(detailRental, 'pending_approval')"
          >
