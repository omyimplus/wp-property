...
definePageMeta({ layout: 'admin', title: 'แก้ไขคำขอซื้อ' })

const route = useRoute()
const id = computed(() => route.params.id as string)

const form = ref<SaleRequestFormData | null>(null)
const sale = ref<SaleRequestListItem | null>(null)
const loading = ref(true)
const saving = ref(false)
const deleting = ref(false)
const errorMessage = ref('')
const successMessage = ref(route.query.created ? 'บันทึกคำขอซื้อสำเร็จ' : '')

const isRejected = computed(() => sale.value?.status === 'rejected')
const isCompleted = computed(() => sale.value?.status === 'completed')
...
async function load() {
  ...
  errorMessage.value = ''
  try {
    const res = await $fetch<{ sale: SaleRequestListItem }>(`/api/admin/sales/${id.value}`)
    sale.value = res.sale
    form.value = saleToFormData(res.sale)
  } catch (e: unknown) {
...
async function onSubmit() {
  ...
  try {
    await $fetch(`/api/admin/sales/${id.value}`, { method: 'PATCH', body: form.value })
...
async function setStatus(status: 'rejected' | 'completed' | 'pending_approval') {
  ...
  if (!confirm(`ตั้งสถานะ「${labels[status]}」สำหรับ ${sale.value?.customer_name}?`)) return
  errorMessage.value = ''
  try {
    await $fetch(`/api/admin/sales/${id.value}/status`, { method: 'PATCH', body: { status } })
    if (status === 'pending_approval') {
      await load()
      successMessage.value = 'ย้อนกลับเป็นรอดำเนินการแล้ว — แก้ไขข้อมูลได้'
    } else {
      await navigateTo(`/admin/sales?status=${status}`)
    }
  } catch (e: unknown) {
    const err = e as { data?: { statusMessage?: string } }
...
async function onDelete() {
  if (!sale.value || sale.value.status !== 'rejected') return
  if (!confirm(`ลบคำขอของ ${sale.value.customer_name} ออกจากระบบถาวร?`)) return
  deleting.value = true
  try {
    await $fetch(`/api/admin/sales/${id.value}`, { method: 'DELETE' })
    await navigateTo('/admin/sales?status=rejected')
  } catch (e: unknown) {
    const err = e as { data?: { statusMessage?: string } }
    ...
    <template v-else-if="form && sale">
      <div class="rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm text-slate-700">
        <p>
          ...
          {{ saleCreatedSourceLabel(sale.created_source) }}
          <LoanCreatedSourceBadge :source="sale.created_source" class="ml-2 inline-flex align-middle" />
        </p>
        <p class="mt-1">
          <span class="font-medium text-slate-900">สร้างโดย:</span>
          {{
            sale.created_source === 'customer_web'
              ? 'ลูกค้าส่งจากเว็บ'
              : staffDisplayName(sale.created_by_name, sale.created_by_email)
          }}
          <span v-if="sale.created_by" class="text-xs text-slate-400">({{ sale.created_by }})</span>
        </p>
        <p v-if="sale.handled_by" class="mt-1">
          <span class="font-medium text-slate-900">จัดการโดย:</span>
          {{ staffDisplayName(sale.handled_by_name, sale.handled_by_email) }}
          <span v-if="sale.handled_at" class="text-xs text-slate-400">
            · {{ new Date(sale.handled_at).toLocaleString('th-TH') }}
          </span>
        </p>
        <p class="mt-1 text-slate-600">
          พื้นที่ต้องการซื้อ: {{ saleDesiredAreaText(sale) }}
        </p>
      </div>

      <div
        v-if="sale.status !== 'pending_approval'"
