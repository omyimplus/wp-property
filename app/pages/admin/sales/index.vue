...
import {
  ...
  saleStatusLabel,
  staffDisplayName,
  type SaleRequestListItem,
  type SaleRequestStatus,
} from '~/types/sale-request'
import type { ThaiLocationValue } from '~/types/thai-location'

definePageMeta({ layout: 'admin', title: 'สนใจซื้อ' })

const filters = reactive(emptySaleFilters())
...
const sales = ref<SaleRequestListItem[]>([])
...
const detailSale = ref<SaleRequestListItem | null>(null)

const rangeLabel = computed(() => {
  if (total.value === 0) return 'ไม่พบรายการ'
  const from = (page.value - 1) * SALE_LIST_PAGE_SIZE + 1
  const to = Math.min(page.value * SALE_LIST_PAGE_SIZE, total.value)
...
function buildQuery() {
  const q: Record<string, string> = {
    ...
    page_size: String(SALE_LIST_PAGE_SIZE),
  }
...
async function load() {
  ...
  try {
    const res = await $fetch<{
      sales: SaleRequestListItem[]
      total: number
      page: number
      total_pages: number
    }>('/api/admin/sales', { query: buildQuery() })
    sales.value = res.sales
    total.value = res.total
    page.value = res.page
    totalPages.value = res.total_pages
    if (detailSale.value) {
      const fresh = res.sales.find(r => r.id === detailSale.value!.id)
      detailSale.value = fresh ?? null
    }
  } catch (e: unknown) {
...
function setStatusTab(status: SaleRequestStatus) {
...
function openDetail(row: SaleRequestListItem) {
...
function createdByText(row: SaleRequestListItem) {
...
function handledByText(row: SaleRequestListItem) {
...
async function patchStatus(
  row: SaleRequestListItem,
  status: SaleRequestStatus,
  event?: Event,
) {
  event?.stopPropagation()
  const confirmMessages: Record<SaleRequestStatus, string> = {
  ...
  try {
    await $fetch(`/api/admin/sales/${row.id}/status`, { method: 'PATCH', body: { status } })
    const successMessages: Record<SaleRequestStatus, string> = {
...
async function onDelete(row: SaleRequestListItem, event?: Event) {
  event?.stopPropagation()
  if (row.status !== 'rejected') return
  if (!confirm(`ลบคำขอของ ${row.customer_name} ออกจากระบบถาวร?`)) return
  actionId.value = row.id
  try {
    await $fetch(`/api/admin/sales/${row.id}`, { method: 'DELETE' })
...
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <h2 class="text-lg font-semibold text-slate-900">สนใจซื้อ</h2>
    ...
    <div class="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <div v-else-if="sales.length === 0" class="p-12 text-center text-slate-500">
        ไม่พบรายการ
      </div>
      <table v-else class="w-full min-w-[640px] text-left text-sm">
        <thead class="border-b border-slate-200 bg-slate-50 text-slate-600">
          <tr>
            ...
            <th class="px-4 py-3 font-medium text-right">ช่วงราคาซื้อ</th>
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          >
            ...
            <td class="px-4 py-3 text-right font-medium tabular-nums">
              {{ saleBudgetText(row.purchase_budget_min, row.purchase_budget_max) }}
            </td>
            <td
    ...
    >
      <div class="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
        <h3 class="mb-4 text-lg font-semibold">กรองคำขอซื้อ</h3>
        <div class="space-y-4">
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              <label class="mb-1 block text-sm font-medium">ราคาซื้อต่ำสุด</label>
              <input v-model="draftFilters.min_price" type="number" min="0" class="w-full rounded-lg border px-3 py-2 text-sm">
            </div>
            <div>
              <label class="mb-1 block text-sm font-medium">ราคาซื้อสูงสุด</label>
              <input v-model="draftFilters.max_price" type="number" min="0" class="w-full rounded-lg border px-3 py-2 text-sm">
            </div>
          </div>
        ...
        <div class="mt-6 flex justify-end gap-2">
          ...
          <button
            ...
            @click="filters.min_price = draftFilters.min_price; filters.max_price = draftFilters.max_price; filters.province = draftFilters.province; filters.district = draftFilters.district; filters.subdistrict = draftFilters.subdistrict; filterDialogOpen = false; page = 1; load()"
          >
            ค้นหา
          </button>
        </div>
    ...
    >
      ...
      >
        <div class="mb-4 flex items-start justify-between gap-3">
          <div>
            <h3 id="sale-detail-title" class="text-lg font-semibold text-slate-900">
              {{ detailSale.customer_name }}
            </h3>
            <div class="mt-1 flex flex-wrap items-center gap-2">
              <RentalRequestStatusBadge :status="detailSale.status" />
              <LoanCreatedSourceBadge :source="detailSale.created_source" />
            </div>
          </div>
          <button
            type="button"
        ...
        <dl class="space-y-3 text-sm">
          <div class="grid grid-cols-[8rem_1fr] gap-2">
            ...
            <dd>{{ detailSale.callback_phone }}</dd>
          </div>
          <div class="grid grid-cols-[8rem_1fr] gap-2">
            <dt class="text-slate-500">ไลน์ / ติดต่อ</dt>
            <dd>{{ detailSale.callback_line }}</dd>
          </div>
          <div class="grid grid-cols-[8rem_1fr] gap-2">
            <dt class="text-slate-500">ช่วงราคาซื้อ</dt>
            <dd class="font-medium">
              {{ saleBudgetText(detailSale.purchase_budget_min, detailSale.purchase_budget_max) }}
            </dd>
          </div>
          <div class="grid grid-cols-[8rem_1fr] gap-2">
          ...
          <div class="grid grid-cols-[8rem_1fr] gap-2 border-t border-slate-100 pt-3">
            ...
            <dd>
              {{ createdByText(detailSale) }}
              <span v-if="detailSale.created_source === 'admin' && detailSale.created_by" class="block text-xs text-slate-400">
                user: {{ detailSale.created_by }}
              </span>
            </dd>
          </div>
          <div
            v-if="detailSale.handled_by"
            class="grid grid-cols-[8rem_1fr] gap-2"
          >
            <dt class="text-slate-500">จัดการโดย</dt>
            <dd>
              {{ handledByText(detailSale) }}
              <span v-if="detailSale.handled_at" class="block text-xs text-slate-400">
                {{ formatHandledAt(detailSale.handled_at) }}
              </span>
            </dd>
          </div>
          <div class="grid grid-cols-[8rem_1fr] gap-2 text-xs text-slate-400">
            <dt>สถานะ</dt>
            <dd>{{ saleStatusLabel(detailSale.status) }}</dd>
          </div>
        </dl>

        <div class="mt-6 flex flex-wrap gap-2 border-t border-slate-100 pt-4">
          <NuxtLink
            v-if="detailSale.status === 'pending_approval'"
            :to="`/admin/sales/${detailSale.id}/edit`"
            class="flex-1 rounded-lg border border-slate-300 py-2 text-center text-sm hover:bg-slate-50"
            @click="closeDetail"
          >
            แก้ไขข้อมูล
          </NuxtLink>
          <template v-if="detailSale.status === 'pending_approval'">
            <button
              type="button"
              class="rounded-lg p-2 text-green-700 hover:bg-green-50 disabled:opacity-40"
              title="ติดต่อแล้ว"
              :disabled="actionId === detailSale.id"
              @click="patchStatus(detailSale, 'completed')"
            >
            ...
            <button
              ...
              title="ไม่อนุมัติ"
              :disabled="actionId === detailSale.id"
              @click="patchStatus(detailSale, 'rejected')"
            >
              <svg class="h-7 w-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
          ...
          <button
            ...
            title="ย้อนกลับเป็นรอดำเนินการ"
            :disabled="actionId === detailSale.id"
            @click="patchStatus(detailSale, 'pending_approval')"
          >
