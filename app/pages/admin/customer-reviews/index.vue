...
import {
  CUSTOMER_REVIEW_STATUS_TABS,
  customerReviewStatusLabel,
  type CustomerReviewListItem,
  type CustomerReviewStatus,
} from '~/types/customer-review'

definePageMeta({ layout: 'admin', title: 'รีวิวจากลูกค้า' })

const statusFilter = ref<CustomerReviewStatus | 'all'>('all')
...
async function load() {
  ...
  try {
    ...
    const res = await $fetch<{ items: CustomerReviewListItem[], total: number }>(
...
function setTab(id: CustomerReviewStatus | 'all') {
  statusFilter.value = id
  load()
}

async function onDelete(row: CustomerReviewListItem) {
  if (!confirm('ลบรีวิวนี้ถาวร?')) return
  actionId.value = row.id
  try {
    await $fetch(`/api/admin/customer-reviews/${row.id}`, { method: 'DELETE' })
...
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <h2 class="text-lg font-semibold text-slate-900">รีวิวจากลูกค้า</h2>
        <p class="mt-1 text-sm text-slate-500">อัปโหลดรูปการ์ดรีวิวสำหรับ carousel หน้าแรก</p>
      </div>
      <NuxtLink
        to="/admin/customer-reviews/create"
    ...
    <div class="flex flex-wrap gap-1 rounded-lg border border-slate-200 bg-white p-1">
      <button
        v-for="tab in CUSTOMER_REVIEW_STATUS_TABS"
        :key="tab.id"
        type="button"
      ...
      <table v-else class="w-full min-w-[560px] text-left text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="row in items" :key="row.id" class="text-slate-800">
            ...
            <td class="px-4 py-3">{{ customerReviewStatusLabel(row.status) }}</td>
            <td class="px-4 py-3 text-center tabular-nums">{{ row.sort_order }}</td>
            <td class="px-4 py-3 text-right">
              <div class="flex justify-end gap-2">
                <NuxtLink
                  :to="`/admin/customer-reviews/${row.id}/edit`"
