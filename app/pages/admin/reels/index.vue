...
definePageMeta({ layout: 'admin', title: 'คลิปที่น่าสนใจ' })

const statusFilter = ref<ReelStatus | 'all'>('all')
const items = ref<ReelListItem[]>([])
...
async function load() {
  ...
  try {
    ...
    const res = await $fetch<{ items: ReelListItem[], total: number }>(
      '/api/admin/reels',
      { query },
    )
...
function setTab(id: ReelStatus | 'all') {
  statusFilter.value = id
  load()
}

async function onDelete(row: ReelListItem) {
  if (!confirm('ลบคลิปนี้ถาวร?')) return
  actionId.value = row.id
  try {
    await $fetch(`/api/admin/reels/${row.id}`, { method: 'DELETE' })
...
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <h2 class="text-lg font-semibold text-slate-900">คลิปที่น่าสนใจ</h2>
        <p class="mt-1 text-sm text-slate-500">อัปโหลดวิดีโอ Reels สำหรับ carousel หน้าแรก</p>
      </div>
      <NuxtLink
        to="/admin/reels/create"
    ...
    <div class="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <table v-else class="w-full min-w-[640px] text-left text-sm">
        <thead class="border-b border-slate-200 bg-slate-50 text-slate-600">
          <tr>
            <th class="px-4 py-3 font-medium">ภาพปก</th>
            <th class="px-4 py-3 font-medium">ลิงก์</th>
          ...
          <tr v-for="row in items" :key="row.id" class="text-slate-800">
            <td class="px-4 py-3">
              <img
                ...
                :src="row.poster_url"
                alt=""
                class="h-16 w-10 rounded object-cover"
              >
            ...
            <td class="px-4 py-3">{{ reelStatusLabel(row.status) }}</td>
            <td class="px-4 py-3 text-center tabular-nums">{{ row.sort_order }}</td>
            <td class="px-4 py-3 text-right">
              <div class="flex justify-end gap-2">
                <NuxtLink
                  :to="`/admin/reels/${row.id}/edit`"
