...
definePageMeta({ layout: 'admin', title: 'บทความ' })
...
async function load() {
  ...
  try {
    ...
    const res = await $fetch<{ items: ArticleListItem[], total: number }>(
      '/api/admin/articles',
      { query },
    )
...
function setTab(id: ArticleStatus | 'all') {
...
async function onDelete(row: ArticleListItem) {
  ...
  try {
    await $fetch(`/api/admin/articles/${row.id}`, { method: 'DELETE' })
...
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <h2 class="text-lg font-semibold text-slate-900">บทความ</h2>
        <p class="mt-1 text-sm text-slate-500">จัดการบทความหน้าบ้าน — รูปหลัก + rich text</p>
      </div>
      <NuxtLink
    ...
    <div class="flex flex-wrap gap-1 rounded-lg border border-slate-200 bg-white p-1">
      <button
        v-for="tab in ARTICLE_STATUS_TABS"
        :key="tab.id"
        type="button"
      ...
      <table v-else class="w-full min-w-[720px] text-left text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="row in items" :key="row.id" class="text-slate-800">
            ...
            <td class="px-4 py-3">{{ articleStatusLabel(row.status) }}</td>
